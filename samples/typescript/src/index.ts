...
async function test() {
  ...
  /*
     ...
     */
  let collection = await table.fetch(['Account', 'User', 'Invoice'], {
    pk: `account#${account.id}`,
  });

  user = await User.update(
    { email: 'roadrunner@acme.com' },
    { set: { 'address.zip': '{"98034"}' } }
  );
  ...
  /*
    ...
    */
  let items = (await User.find({}, { fields: ['email', '_type'] })).map(
    (i) => i.email
  );
  ...
  let transaction = {};
  Invoice.create(
    { product: product.name, count: 1, total: product.price },
    { transaction }
  );
  User.update(
    { email: 'roadrunner@acme.com' },
    { add: { balance: product.price }, transaction }
  );
  Account.update(
    { id: account.id },
    { add: { balance: product.price }, transaction }
  ...
  /*
    ...
    */
  collection = await table.fetch(['Account', 'User', 'Invoice'], {
    pk: `account#${account.id}`,
  });
